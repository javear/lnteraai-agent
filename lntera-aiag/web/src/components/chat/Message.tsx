  ...
  if (message.role === 'user') {
    return (
      <div className="flex animate-fade-in justify-end">
        <div className="max-w-[80%] whitespace-pre-wrap rounded-2xl rounded-br-sm bg-primary px-4 py-2.5 text-[15px] leading-relaxed text-primary-foreground">
          {message.content}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex animate-fade-in gap-3">
      ...
      <div className="min-w-0 flex-1 pt-0.5">
        {message.proactive ? (
          <div className="mb-1.5 inline-flex items-center gap-1.5 rounded-full bg-muted px-2 py-0.5 text-[11px] font-medium text-muted-foreground">
        ...
        {message.model && message.content ? (
          <div className="mt-1.5 text-[11px] text-muted-foreground/70">{message.model}</div>
        ) : null}
      </div>
    </div>
  );
