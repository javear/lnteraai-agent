...
/**
 * The sign-in / sign-up form, shared by the focused `/login` page (`variant="card"`) and the
 * marketing landing hero (`variant="bare"`). Single source of truth for the auth handlers so the
...
export function AuthPanel({
  ...
  variant?: 'card' | 'bare';
  defaultMode?: Mode;
  /** Show the brand mark above the form. Defaults to true only for the card variant. */
  showLogo?: boolean;
  /** Show the "Sign in / Create account" heading. Defaults to true only for the card variant. */
  showHeading?: boolean;
  /** Where to send the user once a session exists. Defaults to the app root. */
  redirectTo?: string;
  className?: string;
}) {
  ...
  const withLogo = showLogo ?? variant === 'card';
  const withHeading = showHeading ?? variant === 'card';
  ...
  const body = (
    <>
      {withLogo ? <Logo size="md" /> : null}
      {withHeading ? (
        <>
          <h1 className={cn('text-2xl font-semibold leading-tight tracking-tight sm:text-3xl', withLogo && 'mt-7')}>
  ...
  if (variant === 'card') {
    return (
      <div
        className={cn(
          'rounded-2xl border bg-card p-6 text-card-foreground shadow-sm sm:p-8',
          className,
        )}
      >
        {body}
