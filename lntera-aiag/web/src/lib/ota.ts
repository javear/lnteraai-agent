...
// Storage. BACKGROUND mode: on launch we (1) call notifyAppReady() so capgo cancels its rollback timer
// for the running bundle, then (2) check the public manifest and, if a newer bundle exists, download it
// in the background (small non-blocking progress toast) and stage it for the NEXT launch. The app stays
// fully usable throughout; nothing blocks or breaks boot. A bundle that fails to boot (never reaches
...
async function checkForUpdate(CapacitorUpdater: Updater): Promise<void> {
  ...
  if (!bundle) {
    // Background download with a small non-blocking progress toast. The app stays fully usable.
  ...
  // Stage for the NEXT cold start (no forced mid-session reload). capgo activates it then; if that boot
  // fails to call notifyAppReady() it reverts to the current (known-good) bundle.
  await CapacitorUpdater.next({ id: bundle.id });

  // Non-blocking heads-up; it also applies automatically on the next launch.
