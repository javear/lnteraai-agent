...
```bash
npm run dev # Mastra Studio + Discord (loads lntera-aiag/.env for unset keys, sets DISCORD_EMBEDDED=1)
npm run discord # Discord-only worker (optional; use if Mastra runs without DISCORD_EMBEDDED)
...
| `src/mastra/processors` | (Optional) Shared [`Processors`](https://mastra.ai/docs/agents/processors) — input/output/error hooks you attach on agents (keep agents thin). |
