...
export function constantTimeEqualString(a: string, b: string): boolean {
  ...
  const da = createHash('sha256').update(a, 'utf8').digest(); // codeql[js/insufficient-password-hash]
  const db = createHash('sha256').update(b, 'utf8').digest(); // codeql[js/insufficient-password-hash]
