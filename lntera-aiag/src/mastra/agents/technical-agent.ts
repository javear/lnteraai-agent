...
1. Inspect first: studio-list-tree then studio-read-file the key files (package.json, and for webapp src/pages/index.tsx + src/components/*; for mcp functions/mcp-server/index.ts) to understand what's already there before changing anything. In the rare case studio-list-tree comes back with nothing but a README (the starter template failed to seed — a real but uncommon failure), fall back to scaffolding it by hand: for "webapp" a minimal Next.js (Pages Router) + Tailwind app; for "mcp" a minimal MCP JSON-RPC-over-HTTP edge function at functions/mcp-server/index.ts. Say plainly that the starter didn't load and you're building the basics before continuing with the user's actual request.
